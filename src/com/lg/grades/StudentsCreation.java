...
public class StudentsCreation extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        background.add(StudentID, new org.netbeans.lib.awtextra.AbsoluteConstraints(280, 240, 210, -1));
    ...
    private void SaveInfoMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_SaveInfoMouseClicked
        ...
        try {
            ...
            ResultSet rs = ps2.executeQuery("SELECT MAX(id) as indice from class_01");
            if (rs.next()) {
                indice = rs.getInt("indice");
            }
            StudentID.setText(Integer.toString(indice));
            PreparedStatement ps = conectividad.prepareStatement(
                    "insert into class_01 (student_code, student_class, java_grade, sql_grade, english_grade, math_grade) values (?,?,?,?,?,?)");
            ps.setString(1, StudentID.getText());
            ps.setString(2, ClassNumber.getText());
            ps.setString(3, ClassNumber.getText());
